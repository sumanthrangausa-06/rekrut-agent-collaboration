...
### 🔴 CRITICAL: Always Use Specialized Agency Agents

**The user has explicitly forbidden creating generic subagents. This is a hard rule.**

When you need to delegate work, you MUST use the specialized agency agents from `agents_list`:
- `devops-automator` for DevOps tasks
- `git-workflow-master` for Git tasks
- `application-security-engineer` for security
- `frontend-developer` for frontend work
- `backend-architect` for backend work
- etc. (210 total agents available)

**Never do this:**
```javascript
// ❌ WRONG — Generic subagent
sessions_spawn({ task: "do something" })
```

**Always do this:**
```javascript
// ✅ CORRECT — Specialized agent
sessions_spawn({ agentId: "devops-automator", task: "specific task" })
```

### Sharing Skills with Subagents
When spawning specialized agents, explicitly tell them to read relevant SKILL.md files. Subagents do not auto-discover skills. Include in their task briefing:
- "Read `~/.openclaw/skills/<skill-name>/SKILL.md` before starting"
- "Use the `wecom-preflight` skill for validation"
- "Follow the `browser-automation` skill for web tasks"

### External Skills
The user has shared GitHub links for additional skills. When available, these should be installed and used. Subagents must also be told to use these external skills.
