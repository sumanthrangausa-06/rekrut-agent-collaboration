#!/bin/bash
# Run a delegated-execution test
# Usage: ./run-test.sh <test-name> [--plugin-dir <path>]
#
# Example:
#   ./run-test.sh go-fractals
#   ./run-test.sh svelte-todo --plugin-dir /path/to/godmode
...
TIMESTAMP=$(date +%s)
OUTPUT_BASE="/tmp/godmode-tests/$TIMESTAMP/delegated-execution"
...
PROMPT="Execute this plan using godmode:delegated-execution. The plan is at: $PLAN_PATH"
