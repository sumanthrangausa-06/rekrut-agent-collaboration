...
TIMESTAMP=$(date +%s)
OUTPUT_DIR="/tmp/godmode-tests/${TIMESTAMP}/explicit-skill-requests/multiturn"
mkdir -p "$OUTPUT_DIR"
...
# Turn 3: The critical test - ask for delegated-execution
echo ">>> Turn 3: Requesting delegated-execution..."
...
if grep -q '"name":"Skill"' "$TURN3_LOG" && grep -qE "$SKILL_PATTERN" "$TURN3_LOG"; then
    echo "PASS: Skill 'delegated-execution' was triggered in Turn 3"
    TRIGGERED=true
else
    echo "FAIL: Skill 'delegated-execution' was NOT triggered in Turn 3"
    TRIGGERED=false
fi
