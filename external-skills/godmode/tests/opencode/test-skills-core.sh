...
# Create skills in personal and godmode locations for testing
mkdir -p "$TEST_HOME/personal-skills/shared-skill"
mkdir -p "$TEST_HOME/godmode-skills/shared-skill"
mkdir -p "$TEST_HOME/godmode-skills/unique-skill"
...
cat > "$TEST_HOME/godmode-skills/shared-skill/SKILL.md" <<'EOF'
---
name: shared-skill
description: GodMode version
---
# GodMode Shared
EOF

cat > "$TEST_HOME/godmode-skills/unique-skill/SKILL.md" <<'EOF'
---
name: unique-skill
description: Only in godmode
---
# Unique
EOF
...
function resolveSkillPath(skillName, godmodeDir, personalDir) {
    const forceGodMode = skillName.startsWith('godmode:');
    const actualSkillName = forceGodMode ? skillName.replace(/^godmode:/, '') : skillName;

    if (!forceGodMode && personalDir) {
    ...
    if (godmodeDir) {
        const godmodePath = path.join(godmodeDir, actualSkillName);
        const godmodeSkillFile = path.join(godmodePath, 'SKILL.md');
        if (fs.existsSync(godmodeSkillFile)) {
            return {
                skillFile: godmodeSkillFile,
                sourceType: 'godmode',
                skillPath: actualSkillName
            };
        }
...
const godmodeDir = '$TEST_HOME/godmode-skills';
const personalDir = '$TEST_HOME/personal-skills';

// Test 1: Shared skill should resolve to personal
const shared = resolveSkillPath('shared-skill', godmodeDir, personalDir);
console.log('SHARED:', JSON.stringify(shared));

// Test 2: godmode: prefix should force godmode
const forced = resolveSkillPath('godmode:shared-skill', godmodeDir, personalDir);
console.log('FORCED:', JSON.stringify(forced));

// Test 3: Unique skill should resolve to godmode
const unique = resolveSkillPath('unique-skill', godmodeDir, personalDir);
console.log('UNIQUE:', JSON.stringify(unique));

// Test 4: Non-existent skill
const notfound = resolveSkillPath('not-a-skill', godmodeDir, personalDir);
console.log('NOTFOUND:', JSON.stringify(notfound));
" 2>&1)

if echo "$result" | grep -q 'SHARED:.*"sourceType":"personal"'; then
    echo "  [PASS] Personal skills shadow godmode skills"
else
...
if echo "$result" | grep -q 'FORCED:.*"sourceType":"godmode"'; then
    echo "  [PASS] godmode: prefix forces godmode resolution"
else
    echo "  [FAIL] godmode: prefix not working"
    exit 1
fi

if echo "$result" | grep -q 'UNIQUE:.*"sourceType":"godmode"'; then
    echo "  [PASS] Unique godmode skills are found"
else
    echo "  [FAIL] Unique godmode skills not found"
    exit 1
fi
