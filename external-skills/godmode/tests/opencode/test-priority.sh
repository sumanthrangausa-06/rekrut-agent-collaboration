...
# Verifies that skills are resolved with correct priority: project > personal > godmode
...
# 1. Create in godmode location (lowest priority)
mkdir -p "$HOME/.config/opencode/godmode/skills/priority-test"
cat > "$HOME/.config/opencode/godmode/skills/priority-test/SKILL.md" <<'EOF'
---
name: priority-test
description: GodMode version of priority test skill
---
# Priority Test Skill (GodMode Version)

This is the GODMODE version of the priority test skill.

PRIORITY_MARKER_GODMODE_VERSION
EOF
...
if [ -f "$HOME/.config/opencode/godmode/skills/priority-test/SKILL.md" ]; then
    echo "  [PASS] GodMode version exists"
else
    echo "  [FAIL] GodMode version missing"
    exit 1
fi
...
# Test 2: Test that personal overrides godmode
echo ""
echo "Test 2: Testing personal > godmode priority..."
...
if echo "$output" | grep -qi "PRIORITY_MARKER_PERSONAL_VERSION"; then
    echo "  [PASS] Personal version loaded (overrides godmode)"
elif echo "$output" | grep -qi "PRIORITY_MARKER_GODMODE_VERSION"; then
    echo "  [FAIL] GodMode version loaded instead of personal"
    exit 1
else
    echo "  [WARN] Could not verify priority marker in output"
    echo "  Output snippet:"
    echo "$output" | grep -i "priority\|personal\|godmode" | head -10
fi

# Test 3: Test that project overrides both personal and godmode
echo ""
echo "Test 3: Testing project > personal > godmode priority..."
...
if echo "$output" | grep -qi "PRIORITY_MARKER_PROJECT_VERSION"; then
    ...
    echo "  [FAIL] GodMode version loaded instead of project"
    exit 1
else
    echo "  [WARN] Could not verify priority marker in output"
    echo "  Output snippet:"
...
# Test 4: Test explicit godmode: prefix bypasses priority
echo ""
echo "Test 4: Testing godmode: prefix forces godmode version..."

cd "$TEST_HOME/test-project"
output=$(timeout 60s opencode run --print-logs "Use the use_skill tool to load godmode:priority-test specifically. Show me the exact content including any PRIORITY_MARKER text." 2>&1) || {
    exit_code=$?
    if [ $exit_code -eq 124 ]; then
        echo "  [FAIL] OpenCode timed out after 60s"
        exit 1
...
if echo "$output" | grep -qi "PRIORITY_MARKER_GODMODE_VERSION"; then
    echo "  [PASS] godmode: prefix correctly forces godmode version"
elif echo "$output" | grep -qi "PRIORITY_MARKER_PROJECT_VERSION\|PRIORITY_MARKER_PERSONAL_VERSION"; then
    echo "  [FAIL] godmode: prefix did not force godmode version"
    exit 1
else
    echo "  [WARN] Could not verify priority marker in output"
fi
