...
if echo "$output" | grep -qi "godmode:intent-discovery\|godmode:activation\|Available skills"; then
    echo "  [PASS] find_skills tool discovered godmode skills"
else
...
# Test 3: Test use_skill with godmode: prefix
echo ""
echo "Test 3: Testing use_skill with godmode: prefix..."
echo "  Running opencode with godmode:intent-discovery skill..."

output=$(timeout 60s opencode run --print-logs "Use the use_skill tool to load godmode:intent-discovery and tell me the first few lines of what you received." 2>&1) || {
    exit_code=$?
    if [ $exit_code -eq 124 ]; then
        echo "  [FAIL] OpenCode timed out after 60s"
        exit 1
...
# Check for expected content from intent-discovery skill
if echo "$output" | grep -qi "intent-discovery\|Launching skill\|skill.*loaded"; then
    echo "  [PASS] use_skill loaded godmode:intent-discovery skill"
else
    echo "  [FAIL] use_skill did not load godmode:intent-discovery correctly"
    echo "  Output was:"
    echo "$output" | head -50
    exit 1
fi
