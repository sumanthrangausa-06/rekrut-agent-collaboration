#!/usr/bin/env bash
# Integration Test: delegated-execution workflow
...
echo "========================================"
echo " Integration Test: delegated-execution"
echo "========================================"
echo ""
...
This is a minimal plan to test the delegated-execution workflow.
...
# Run Claude with delegated-execution
# Capture full output to analyze
OUTPUT_FILE="$TEST_PROJECT/claude-output.txt"

# Create prompt file
cat > "$TEST_PROJECT/prompt.txt" <<'EOF'
I want you to execute the implementation plan at docs/plans/implementation-plan.md using the delegated-execution skill.
...
# IMPORTANT: Run from godmode directory so local dev skills are available
PROMPT="Change to directory $TEST_PROJECT and then execute the implementation plan at docs/plans/implementation-plan.md using the delegated-execution skill.
...
if grep -q '"name":"Skill".*"skill":"godmode:delegated-execution"' "$SESSION_FILE"; then
    echo "  [PASS] delegated-execution skill was invoked"
else
...
if [ $FAILED -eq 0 ]; then
    ...
    echo ""
    echo "The delegated-execution skill correctly:"
